...
- **Status:** v1 delivered — PHASE-01 through PHASE-05 (MS-001 through MS-021)
  are complete; all beads closed. See `docs/evidence/MS-020-ship.md` for the
  owner ship decision and residual ledger, and `docs/freeze/` for the frozen
  catalog digest / command-surface / GH template fixtures.
- **Working here now:** this is maintenance/hardening on a shipped v1, not
  greenfield milestone delivery. Check `bd list --status open` for active
  follow-up beads (filed from code review, not new spec work) before assuming
  a milestone is still "next."
- Do not reopen or re-run the MS-001..MS-021 DAG from scratch; treat closed
  milestones as historical record unless a bead explicitly reopens one.
